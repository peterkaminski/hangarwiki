...
- **Username derivation:** `email-local-part`, lowercased, scrubbed to `[a-z0-9_-]`, truncated to 40. On collision, append `-2`, `-3`, … via a server-side counter. Stored on the `users` row as `forge_username` so HangarWiki doesn't have to re-derive. Forgejo username uniqueness is the source of truth, but we mirror it to avoid extra API calls.
